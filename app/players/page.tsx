  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-sm text-slate-500">Số người chơi</div>
        ...
        <SectionCard title="Danh sách người chơi">
          {sortedPlayers.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-center">
              ...
              <div className="text-base font-semibold">Chưa có người chơi</div>
                    ...
                    <Link href={`/players/${player.id}`} className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 grid gap-2 text-xs md:grid-cols-2">
                        <div className="rounded-xl bg-white px-3 py-2 text-slate-600">
                          Normal — Rating: {player.ratingNormal} • W:{" "}
                          {player.winsNormal} • L: {player.lossesNormal} • M:{" "}
                          {player.matchesNormal}
                        </div>
                        <div className="rounded-xl bg-white px-3 py-2 text-slate-600">
                          Team — Rating: {player.ratingTeam} • W: {player.winsTeam} •
                          L: {player.lossesTeam} • M: {player.matchesTeam}
                        </div>
                      </div>
                    </Link>
        ...
        <SectionCard title="Ghi chú">
          <div className="space-y-2 text-sm text-slate-600">
            <div>• Dữ liệu người chơi đang lưu bằng localStorage trên máy hiện tại.</div>
            <div>• Bấm vào từng người chơi để xem thống kê chi tiết.</div>
      ...
      <ConfirmDialog
        ...
        description={
          deleteTarget
            ? `Bạn có chắc muốn xoá ${deleteTarget.name} khỏi danh sách người chơi không?`
            : ""
        }
        confirmText="Xoá"
        cancelText="Huỷ"
      ...
      <ConfirmDialog
        ...
        description="Thao tác này sẽ thay toàn bộ danh sách hiện tại bằng 8 người chơi mẫu ban đầu."
